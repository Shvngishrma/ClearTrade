import { DocumentGenerationError, generateShippingBillPdfBuffer } from "@/lib/documentPdfService"
...
function toErrorResponse(error: unknown) {
  ...
  return NextResponse.json(
    {
      error: "SHIPPING_BILL_PDF_GENERATION_FAILED",
      message: error instanceof Error ? error.message : "Unable to generate shipping bill PDF",
    },
    { status: 500 }
  )
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const pdf = await generateShippingBillPdfBuffer(invoiceId, { usage })
