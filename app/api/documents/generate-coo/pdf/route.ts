import { DocumentGenerationError, generateCertificateOfOriginPdfBuffer } from "@/lib/documentPdfService"
...
function toErrorResponse(error: unknown) {
  ...
  return NextResponse.json(
    {
      error: "CERTIFICATE_OF_ORIGIN_PDF_GENERATION_FAILED",
      message: error instanceof Error ? error.message : "Unable to generate certificate of origin PDF",
    },
    { status: 500 }
  )
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const pdf = await generateCertificateOfOriginPdfBuffer(invoiceId, { usage })
