import { DocumentGenerationError, generateLetterOfCreditPdfBuffer } from "@/lib/documentPdfService"
...
function toErrorResponse(error: unknown) {
  ...
  return NextResponse.json(
    {
      error: "LC_PDF_GENERATION_FAILED",
      message: error instanceof Error ? error.message : "Unable to generate letter of credit PDF",
    },
    { status: 500 }
  )
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const pdf = await generateLetterOfCreditPdfBuffer(invoiceId, { usage })
